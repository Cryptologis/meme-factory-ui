...
export default function BondingCurveProgress({
  ...
  const currentSol = Number(virtualSolReserves.toString()) / 1e9;
  const tokensAvailable = Number(virtualTokenReserves.toString()) / 1e9;
  const totalTokens = Number(totalSupply.toString()) / 1e9;
  
  // Initial virtual SOL reserves
  const initialSol = 30;
  
  // Initial virtual token reserves
  const initialVirtualTokenReserves = new BN(1_073_000_000_000);
  ...
  const tokensSold = Math.max(0, Number(tokensSoldBN.toString()) / 1e9);
