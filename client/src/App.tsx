  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-background">
        <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 border-b sticky top-0 z-50 backdrop-blur-xl bg-background/80">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-6">
              <a href="/" className={`text-sm font-medium transition-colors hover:text-primary ${location === "/" ? "text-foreground" : "text-muted-foreground"}`}>
                Marketplace
              </a>
              <a href="/create" className={`text-sm font-medium transition-colors hover:text-primary ${location === "/create" ? "text-foreground" : "text-muted-foreground"}`}>
                Create Token
              </a>
          ...
          <div className="flex items-center gap-4">
            ...
            {connected ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleDisconnect} className="px-4 py-2 text-sm font-medium rounded-lg border hover:bg-accent transition-colors">
                  Disconnect
                </button>
              </div>
            ) : (
              <button onClick={handleConnectWallet} className="px-4 py-2 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
                Connect Wallet
              </button>
            )}
          </div>
        ...
        <Router />
        <Toaster />
        <WalletConnectionModal open={walletModalOpen} onClose={() => setWalletModalOpen(false)} onSelectWallet={handleSelectWallet} />
      </div>
    </TooltipProvider>
  );
}
