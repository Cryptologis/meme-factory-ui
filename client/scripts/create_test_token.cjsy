...
async function main() {
  ...
  if (!rpcUrl || !keypairPath || !name || !symbol) {
    console.error("Usage: node client/scripts/create_test_token.cjs <RPC_URL> <KEYPAIR_JSON_PATH> \"<TOKEN_NAME>\" \"<SYMBOL>\"");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Failed to read or parse keypair:", e.message);
    process.exit(1);
  }
  ...
  // Load IDL (ensure client/src/lib/meme_chain.json exists)
  let idl;
  try {
    const idl = require('../src/lib/meme_chain.json');
  } catch (e) {
    console.error("Failed to load IDL at client/src/lib/meme_chain.json:", e.message);
    process.exit(1);
  }

  // Program ID used by the UI (replace if different)
  ...
  try {
    ...
    console.error("Failed to fetch protocol account:", e.message);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Error creating token:", err && err.toString ? err.toString() : err);
    process.exit(1);
  }
}
